...
RSpec.describe MissingController, type: :controller do
  ...
  let(:valid_attributes) { { user_id: user.id, name: 'John Doe', age: 31, gender: 'Male', ethnicity: 'Caucasian', date_birth: '1991-01-01' } }
  let(:invalid_attributes) { { user_id: nil, name: '', age: nil } }
  ...
  describe "PUT #update" do
    let(:missing) { create(:missing_person, valid_attributes) }

    context "with valid params" do
      let(:new_attributes) { { name: 'Jane Doe' } }

      it "updates the requested missing person" do
        put :update, params: { id: missing.id, missing_person: new_attributes }
        missing.reload
        expect(missing.name).to eq('Jane Doe')
      end
    ...
    context "when the missing person does not exist" do
      it "returns an unprocessable entity status" do
        put :update, params: { id: 99999, missing_person: valid_attributes }
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
  end
  ...
  describe "DELETE #destroy" do
    ...
    it "deletes the missing person" do
      expect {
        delete :destroy, params: { id: missing.id }
      }.to change(MissingPerson, :count).by(-1)
    end
