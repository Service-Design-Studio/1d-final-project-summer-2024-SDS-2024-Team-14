...
RSpec.describe MissingController, type: :controller do
  ...
  let(:valid_attributes) { { name: 'John Doe', age: 30, gender: 'Male', ethnicity: 'Caucasian', date_of_birth: '1994-08-06' } }
  let(:invalid_attributes) { { name: '', age: nil, gender: nil, ethnicity: nil, date_of_birth: nil } }

  describe 'POST #create' do
    context 'with valid params' do
      it 'creates a new MissingPerson' do
        expect {
          post :create, params: { user_id: user.id, missing_person: valid_attributes }
        }.to change(MissingPerson, :count).by(1)
      end
    ...
    context 'with invalid params' do
      it 'does not create a new MissingPerson' do
        expect {
          post :create, params: { user_id: user.id, missing_person: invalid_attributes }
        }.to change(MissingPerson, :count).by(0)
      end
  ...
  describe 'DELETE #destroy' do
    ...
    it 'returns error if not found' do
      ...
      expect(response).to have_http_status(:unprocessable_entity)
      expect(response.body).to include('Missing person not found')
    end
  end
  ...
  describe 'PUT #update' do
    context 'with valid params' do
      ...
      it 'updates the requested missing_person' do
        put :update, params: { id: missing_person.id, missing_person: new_attributes }
        missing_person.reload
        expect(missing_person.name).to eq('Jane Doe')
      end
    ...
    context 'with invalid params' do
      it 'returns error if not found' do
        put :update, params: { id: 'invalid_id', missing_person: new_attributes }
        expect(response).to have_http_status(:unprocessable_entity)
  ...
  describe 'GET #photo' do
    context 'with attached photo' do
      it 'returns the photo URL' do
        missing_person.photo.attach(fixture_file_upload(Rails.root.join('spec', 'fixtures', 'files', 'photo.jpg'), 'image/jpg'))
  ...
  describe 'GET #show' do
    it 'returns the missing people for the user' do
      get :show, params: { id: user.id }
      expect(response).to have_http_status(:ok)
      expect(response.body).to include('missing_people')
    end
