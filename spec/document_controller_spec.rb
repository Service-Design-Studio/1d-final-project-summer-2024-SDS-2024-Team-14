    ...
    #     })
      stub_request(:post, /generativelanguage.googleapis.com/).
        to_return(lambda { |request|
          if request.body.include?('unsupported file type')
            { status: 422, body: { error: "Unsupported file type" }.to_json }
          else
            { status: 200, body: { candidates: [{ content: { text: "Processed text content" } }] }.to_json }
          end
        })
...
#         post :create, params: { id: user.id, category: category, files: [file], language: 'english' }
#         expect(response).to have_http_status(:ok)
#       end
...
#       files = [fixture_file_upload('spec/fixtures/mockresume.pdf'), fixture_file_upload('spec/fixtures/mockeng.pdf')]
...
describe 'POST #create' do
    let(:valid_attributes) {
      { id: @user.id, category: 'education', files: [fixture_file_upload('spec/fixtures/mockeng.pdf')] }
    }
    let(:invalid_attributes) {
      { id: nil, category: 'education', files: [] }
    }

    context 'with valid parameters' do
      it 'creates a new document and processes it correctly' do
        puts response.body
        expect {
      ...
      it 'handles multiple file uploads' do
        files = [fixture_file_upload('spec/fixtures/mockresume.pdf'), fixture_file_upload('spec/fixtures/mockeng.pdf')]
        expect {
          post :create, params: { id: @user.id, category: 'education', files: files }
        }.to change(Document, :count).by(files.size)
        expect(response).to have_http_status(:ok)
      end
    end
    ...
    context 'with invalid parameters' do
      it 'returns an error response' do
        get :retrieve, params: { id: 999999 }
        expect(response).to have_http_status(:unprocessable_entity)
        expect(JSON.parse(response.body)['message']).to eq("User does not exist")
      end
    

      it 'returns an error response when no file is provided' do
  ...
  describe 'GET #retrieve' do
  let!(:document) { Document.create(id: 1, user: @user, name: 'Test Document', category: 'education') }

    context 'with valid parameters' do
      it 'returns a list of documents for a user' do
        get :retrieve, params: { id: @user.id, category: 'education' }
        expect(response).to have_http_status(:ok)
        expect(JSON.parse(response.body)['documents']).not_to be_empty
      end
  ...
  let!(:document) { Document.create(id: 1, user: @user, name: 'Test Document', status: 'Pending') }
    ...
    context 'when document exists and status changes' do
      it 'updates the status of a document to approved' do
        expect {
      put :status, params: { id: document.id, status: 'approved', message: 'Approved successfully' }
    }.to change { document.reload.status }.from('pending').to('approved')
    expect(response).to have_http_status(:ok)
  end
    end
    ...
    context 'when document does not exist' do
      it 'returns an error status' do
        put :status, params: { id: 'nonexistent', status: 'approved' }
        expect(response).to have_http_status(:unprocessable_entity)
        expect(JSON.parse(response.body)['message']).to eq('Document does not exist')
      end
    end
