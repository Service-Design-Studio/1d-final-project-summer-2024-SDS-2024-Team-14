...
export default function NotificationPage({ open, setOpen, unread, setUnread }) {
    ...
    return (
        <div className={`${open ? `w-screen md:w-fit lg:w-4/12 md:h-screen xl:3/12 shadow-md opacity-100 ease-in` : `w-0 opacity-0 ease-out`} md:min-h-fit absolute h-screen md:h-auto max-h-screen overflow-y-scroll overflow-x-clip bg-white z-40 md:right-0 md:top-10 top-0 pointer-events-auto transition-all duration-300`}>
            <div 
